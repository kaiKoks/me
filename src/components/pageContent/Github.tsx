      ...
      {isError ? (
        ...
        <div className="flex gap-3 justify-center flex-wrap">
          {Array.from({ length: 3 }).map((_, index) => (
            <div
              key={index}
              className="bg-gray-50/70 dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 rounded-lg shadow-sm min-w-[90%] md:min-w-57  h-35.5 "
            >
              <div className="p-6">
            ...
            >
              <div className="px-6 pt-6 pb-3 flex-1 ">
                <div className="flex items-start justify-between mb-1">
                  <h3 className="text-lg text-nowrap font-semibold pr-2 group-hover:text-green-600 dark:group-hover:text-green-500  transition-colors duration-150">
                    {repo.name}
                  </h3>
                  <a
                    href={repo.html_url}
              ...
              <div className="px-5 ">
                <div className="flex flex-wrap gap-2 mb-3">
                  ...
                  {repo.topics.slice(0, 2).map((topic) => (
                    <span
                      key={topic}
                      className="px-2 py-1 border border-gray-300  text-xs rounded-full"
                    >
                      {topic}
                    </span>
                  ))}
